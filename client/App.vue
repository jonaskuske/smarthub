<template>
  <div id="root">
    ...
    <button type="button" @click="startSys">Alarmanlage aktivieren</button>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    socketClient.on(EVENT_TYPES.TEMPERATUR, temp => {
      console.log(temp)
      // this.name = EVENT_TYPES.TEMPERATUR.
    })
  },
  methods: {
    turnOn() {
    ...
    startSys() {
      socketClient.emit(EVENT_TYPES.START_SYS)
    },
    handleSuccess() {
